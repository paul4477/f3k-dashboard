...
export default {
  ...
  computed: {
    list () {return this.$store.state.events.list},
    isLoading () {return this.$store.state.events.isLoading},
    populated () {return this.$store.state.events.populated},
    },

created() {
  console.log(this.populated)

  if ( !this.populated ) { this.$store.dispatch('events/populate_events') }
/*     this.isLoading = true;
